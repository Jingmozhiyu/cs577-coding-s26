...
public class Range {
    public static class SegmentTree {
        ...
        private void pushDown(int node, int l, int r) {
            ...
            if (isSet[node]) {
                ...
                lazyAdd[leftNode] = 0;
                tree[leftNode] = val * (mid - l + 1);


                isSet[rightNode] = true;
                lazySet[rightNode] = val;
                lazyAdd[rightNode] = 0;
                tree[rightNode] = val * (r - mid);
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < q; i++) {
            int op = sc.nextInt();
            if (op == 1) {
                segTree.updateAdd(sc.nextInt() - 1, sc.nextInt() - 1, sc.nextInt());
            } else if (op == 2) {
                segTree.updateSet(sc.nextInt() - 1, sc.nextInt() - 1, sc.nextInt());
            } else if (op == 3) {
                out.println(segTree.query(sc.nextInt() - 1, sc.nextInt() - 1));
            }
        }
        out.flush();
    }
